...
namespace QLKho
{
    ...
    {
        public frmChiTietPhieuXuat(string mapx, string ngayxuat, string tnn, string dcn, string makho)
        {
            InitializeComponent();
            txtMaPX.Text = mapx;
        ...
        private void LoadData()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    string query = "select hh.MaHH, hh.TenHH, hh.DVT, hh.Gia, ctpx.SoLuong, SUM(ctpx.SoLuong*hh.Gia)as TongTien from ChiTietPhieuXuat as ctpx join HangHoa as hh on ctpx.MaHH = hh.MaHH join PhieuXuat as px on ctpx.MaPX = px.MaPX where ctpx.MaPX = @mapx and ctpx.MaHH = hh.MaHH GROUP BY hh.MaHH, hh.TenHH, hh.DVT, hh.Gia, ctpx.SoLuong ";
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string checkQuery = "SELECT COUNT(*) FROM ChiTietPhieuXuat WHERE MaHH = @MaHH and MaPX = @mapx";
                using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                {
                    checkCommand.Parameters.AddWithValue("@MaHH", mahh);
                    checkCommand.Parameters.AddWithValue("@mapx", mapx);
                    ...
                    if (rowCount > 0)
                    {
                        ...
                        string updateQuery = "UPDATE ChiTietPhieuXuat SET SoLuong = SoLuong + @SoLuong WHERE MaHH = @MaHH";
                    ...
                    else
                    {
                        ...
                        string insertQuery = "INSERT INTO ChiTietPhieuXuat (MaPX, MaHH, SoLuong) VALUES (@mapx, @mahh, @soluong)";
                        using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                        {
                            insertCommand.Parameters.AddWithValue("@mahh", mahh);
                            insertCommand.Parameters.AddWithValue("@soluong", soluong);
                            insertCommand.Parameters.AddWithValue("@mapx", mapx);
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    string checkQuery = "SELECT SoLuong FROM ChiTietPhieuXuat WHERE MaPX = @mapx AND MaHH = @mahh";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        checkCommand.Parameters.AddWithValue("@mapx", mapx);
                        checkCommand.Parameters.AddWithValue("@mahh", mahh);
                        ...
                        if (soLuongNhapHienTai <= soluong)
                        {
                            ...
                            string deleteQuery = "DELETE FROM ChiTietPhieuXuat WHERE MaPX = @mapx AND MaHH = @mahh";
                            using (SqlCommand deleteCommand = new SqlCommand(deleteQuery, connection))
                            {
                                deleteCommand.Parameters.AddWithValue("@mapx", mapx);
                                ...
                                if (rowsAffected > 0)
                                {
                                    MessageBox.Show("Xóa hàng hóa khỏi phiếu xuất thành công!");
                                }
                                else
                                {
                                    MessageBox.Show("Không thể xóa hàng hóa khỏi phiếu xuất. Vui lòng kiểm tra lại mã hàng hóa và mã phiếu nhập.");
                                }
                            }
                        }
                        else
                ...
                {
                    MessageBox.Show("Lỗi khi xóa hàng hóa khỏi phiếu xuất: " + ex.Message);
                }
                finally
                {
                    dgvThongTinMatHang.Rows.Clear();
